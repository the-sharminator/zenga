class JengaTower {  
    ...
    void letsGetPhyiscal() {
        ...
        for (int pivot = int(towerSize.y)-1; pivot >= 0; pivot--) {
            int pivotStructure = unbinary(str(structure[pivot][0]) + str(structure[pivot][(pivot % 2 == 1 ? 1 : 3)]) + str(structure[pivot][(pivot % 2 == 1 ? 2 : 6)]));
            pivotStructure = (pivotStructure == 3 || pivotStructure == 6 ? 2: pivotStructure);
            if (pivotStructure == 1 || pivotStructure == 2 || pivotStructure == 4) {
